...
Tests are not done following the code, they are done before and versioned as source of truth of expecting behavior and documented features.
...
- **Car:** `CarHistoryModel.DeltaMileage` is real user-entered data (read off the trip computer), not derived - `CarMetricsService` cross-checks it against consecutive `Mileage` readings to flag typos/skipped entries.
  ...
  - **A refuel's station is a reference, not free text** (`CarHistoryModel.StationId` -> `car_station`, owner-less by the same deliberate exception to "every collection has `owner_id`" as the `*_reference` collections: a station at an address is a public fact).
    It replaced a `station.brand_name` embedded on every entry *plus* that entry's own copy of the station's city/postal code/coordinates - so a refuel no longer carries a location at all, and `CarHistoryController.OnListMappedAsync` hydrates `StationBrandName`/`StationCity` for display through `CarStationHydrator`, one batched lookup per page.
    Maintenance/Other entries keep their own location and their free-text `Garage`: they have no station to inherit one from, and a garage carries no location of its own here, so there is nothing to de-duplicate.
  ...
  - **Members create stations inline, admins curate them.** The natural key is normalized brand name + normalized city + postal code (`FindByNaturalKeyAsync`, unique index); `POST /api/car-stations` is find-or-create, so the picker can post on every keystroke-committed name without minting duplicates.
    A member refuelling somewhere new must never wait on an admin - same "act on what's confident, leave the rest for a human" split as reference auto-resolution and its admin queue.
    `/admin/car-stations` (`AdminOnly`) is where a station gains its real city/coordinates and where duplicates are merged.
    `city_normalized` is `""` (never null/missing) for a cityless station, or the unique index would collapse every one of them onto a single key - the same null-or-empty trap as the reference collections' unresolved filters.
  - **A station in use cannot be deleted, only merged** (409 naming the count): a station is only reachable through an entry's `StationId`, so deleting one blanks every referencing refuel's location with nothing to recover it from.
    The merge re-points every entry (`RepointStationAsync`) **before** deleting the absorbed document, exactly like the video-game reference merge, and fills only the survivor's *gaps* ("never overwrite with nothing").
  ...
  - **The refuel form checks the total against what was pumped, and only warns** (`Common.System/RefuelCostCheck`, live as the user types - which is why it sits in the one project `BlazorApp` and `Domain` both see, since `BlazorApp` never references `Domain`).
    Fuel type, volume, unit price and cost sit in one row so the three numbers that must agree are read together.
    A receipt legitimately differs from the pump (a car wash on the same ticket, a loyalty discount), so it never auto-fills or blocks; a *missing* cost when one is computable does count as a mismatch.
    The tolerance is derived from what the pump's display rounds away (volume to 0.01, unit price to 0.001) rather than being a flat constant - a flat one either false-positives on a big tank or waves a real error through on a small one.
  - `CarHistoryDto.FuelCategory` ("SP95-E10") is offered through `SuggestInput` fed by `GET /api/car-history/fuel-categories`, the same "suggest what you've already typed" shape as gear categories - both now over the one `MongoDbRepositoryBase.FindDistinctValuesAsync`.
...
- **Automatic resolution confirms a *named* match; it never trusts that a provider returned one row** (`ReferenceMatchRules`, shared by all five domains).
  Counting rows reads a property of the *search* as a property of the *answer*, and it is wrong in both directions - measured live on every provider here.
  It refuses ordinary titles, because TMDB's search is fuzzy rather than exact (`The Bear` + 2022 returns 8 results, `Dark` + 2017 returns 15, `Heat` + 1995 returns 14, the right answer first and exactly named in all three), and it links titles nobody compared (`search/tv?query=Fallout&first_air_date_year=2025` returns exactly one result and it is "Thirst Trap: The Fame. The Fantasy. The Fallout.").
  ...
  - **There are two identity shapes, and they are a real difference between domains rather than an inconsistency to flatten.**
    ...
    A book and an album are a title **plus a creator** (`ConfirmedCreatorMatches`): Google Books answers `intitle:The Hobbit+inauthor:Tolkien` with 300 volumes spanning 1981 to 2012, all one book, so the year is a *tie-break inside the ranking and never a filter*, and several confirmed candidates are printings of one work rather than an ambiguity - the best is linked instead of the set being refused.
    Requiring a year for books would refuse every edition of every work; that is why the video game rule was deliberately not copied there.
  - **An identity field is mandatory for any automatic link, in every domain** (owner's rule): a year for films, shows and games, a creator for books and albums.
    ...
    The accepted trade is the same one video games already made: strictly fewer unattended links, and no wrong ones.
  - **Candidates are not required to agree with *each other* about the creator, only with the one the tenant supplied.**
    Google Books credits one book to "J.R.R. Tolkien", "J. R. R. Tolkien" and "John Ronald Reuel Tolkien"; demanding they agree reads one author as three and refuses exactly the case the rule exists for.
  - **An exactly-spelled title beats a loosely-matched one** (`ConfirmedMatches`' two tiers).
    `NormalizeLoose` drops "the" and every parenthesised group, which is right when comparing one provider's canonical title to another's and wrong against tenant-typed text: TMDB answers `Alien` (1979) with both *Alien* and *The Alien*, two different films of the same year that loose matching cannot separate.
    Falling back to the loose tier only when nothing matches exactly is what still links `Shogun` to TMDB's *Shōgun*.
- **A hard year filter must never be the only query asked** - the "an optional narrowing parameter must never silently zero out results a broader search would find" rule, and which providers it applies to was measured one by one.
...
- **IGDB** is unlike every other client here in three ways, all handled outside the client so it stays an ordinary typed `HttpClient`.
  ...
  - **`search` is relevance-ordered and genuinely noisy**: "Half-Life 2" returns three MMod variants above the canonical game, which is why admin search shows several candidates and automatic resolution only ever acts on a single one.
    Its results are never returned raw - see `VideoGameReferenceClientBase` above for the pool-and-rank policy that puts the canonical game first in all of those cases.
...
- **Open Library** never sends `year` as a server-side filter: `first_publish_year` is the *work's* original year, not a tenant's edition, so filtering by it returns zero relevant results ("Killing Floor" + 2016 reprint).
  ...
  **Discogs' year filter turned out to be the same trap** and is no longer sent alone: confirmed live, `q=Kid A&artist=Radiohead&year=2001` returns exactly one master and it is *Amnesiac*, while the same query without the year finds the album at once. `DiscogsClient` now asks with and without it and unions the two, the year surviving as a ranking key. This file previously recorded the opposite ("no equivalent bug has been found for it"), which is why it is called out rather than quietly edited. No video game client sends one any more (see `VideoGameReferenceClientBase` above - RAWG's `&dates=` was the same trap, and IGDB never had one).
...
- **The video game search policy is written once in `VideoGameReferenceClientBase`**, the same split as `BookReferenceClientBase`: a client supplies only `SearchByRelevanceAsync(title, limit)` (plus its other query shapes), and `SearchGamesAsync` asks `FindGamesByExactTitleAsync` **first**, unions the relevance pool, and ranks what came back.
  A provider's relevance ranking is not an answer on its own here and truncating it to what gets displayed is what loses the game: confirmed live, `search "Code Vein"` puts the 2019 game **sixth** behind its own sequel, three DLC packs and a season pass, and `search "Resident Evil"` opens with six bundles and archive re-releases before the seven games actually named that.
  So the exact-name lookup guarantees a perfect title match is always among the candidates (relevance can bury it arbitrarily deep, or never return it - "Marvel's Avengers"), the relevance query reads `RelevancePoolSize` (50) to fill a five-slot picker, and the ranking chooses.
  - **`ReferenceMatchRules` is the single declaration of "is this candidate that work", read by every surface in every domain that answers it** (it began as `VideoGameMatchRules` and was hoisted once the other four domains needed the same rule): the search ranking above, `TryAutoResolveVideoGameAsync`'s confidence check, and `TryAdoptDefaultVideoGameProviderAsync`/the admin reconciliation row.
    They were three separate opinions about one question, of which only adoption's had been thought through.
    - `OrderByBestMatch` orders every candidate list a human sees - names the work (`TitleNormalizer.LooselyEqual`), then the year, then title distance (an edition/DLC/bundle is the game plus something, so the shortest is the closest to what was asked for), then title for a total order.
    ...
    - `YearRank` is **three-state, not a boolean**: the requested year, then a candidate the provider reports **no** year for, then a contradicting year.
      Folding "unknown" into "contradicts" discards the right answer when a provider has no date; folding it into "agrees" counts it as confirmation - which floated IGDB's dateless "Resident Evil" above the 2002 remake, and let its dateless "Resident Evil 2" block automatic resolution of the 2019 one.
    - `ConfirmedMatches` returns **only the best year tier any candidate reaches**, which is what makes the year decisive rather than decorative: a search for "Resident Evil 2" (2019) finds the 2019 game *and* an undated namesake, and reporting both as an ambiguity the year has already settled would strand it forever - the undated entry has no year to ever be told apart by.
      A contradicting year never confirms however alone the candidate is: that is precisely where a title match is a remake or a same-named sequel.
  - **`TryAutoResolveVideoGameAsync` links on a single *confirmed* match, not on "the provider returned exactly one result"** - the old rule read a property of the search as a property of the answer, and was wrong both ways.
    It linked whatever came back when a query happened to be narrow (IGDB answers "NieR:Automata" with a single "Untitled NieR:Automata Project"), and it refused every title with namesakes however unambiguous the year made it - IGDB holds **eight** games named exactly "Resident Evil 2", so the 2019 remake could never resolve automatically though only one of the eight is from 2019.
    Three of those eight are 1998, so that year stays a human decision: the year narrows, it never invents certainty.
  - **A year is required for any automatic video game link** (owner's rule): `TryAutoResolveVideoGameAsync` returns immediately when `year is null`, and this domain alone has **no** title-only local fallback in `TryLinkExistingVideoGameReferenceAsync`.
    Not even a title the provider holds exactly one game under links without one - the cases where a bare title identifies one game are not worth a rule that silently links the wrong thing everywhere else.
  - **The detail page's "check for reference match" escalates to the provider when nothing local matches** (`LinkVideoGameReferenceAsync`), video games only.
    Without it the year is a one-shot chance taken at creation: an item created before its year was known writes no reference at all, so the local-only lookup has nothing to find however correct the title and year later become - while the button's own tooltip promises "Edit the title or year below, then check again".
    It adds no guessing (it links exactly what resolution on create would have). **All five domains now have it** (`LinkTvShowReferenceAsync`/`LinkMovieReferenceAsync`/`LinkBookReferenceAsync`/`LinkAlbumReferenceAsync`), since the dead end is identical wherever the identity field is supplied after creation - a year for films and shows, a creator for books and albums.
  - **Editing a field never searches by itself.** There is no update hook: the user clicks the button, and that is the only thing that re-resolves.
  - **A record update may never change a reference link, and the server enforces that rather than trusting the client** (`DataCrudControllerBase.PreserveServerOwnedFieldsAsync`, over the `IReferenceLinkedModel` the five reference-linked models implement).
    A PUT is a full replace, so every field the client sends wins - `OwnerId` was already overwritten from the caller's claims for exactly this reason, and the reference link is the other case.
    ...
    **This was the root cause behind a long run of "it doesn't match, but if I click refresh it matches" reports**: the match had happened, the next edit silently undid it, and the button resolved it again.
    Nothing about the page changes when a save removes a link, which is what kept it invisible; only an e2e assertion that the link is *unchanged* across an unrelated edit catches it.
    The link is still written freely by resolution, the detail page's check and an admin unlink - those go straight through a repository, not through the CRUD controller.
  - The journey is covered by `VideoGameReferenceMatchSmokeTest` (three Playwright scenarios, real UI through real IGDB), because every regression here got past a green unit suite - the rule was right each time and the path around it was not.
    **Those tests only mean anything while no reference is left behind**, so they delete every one they create (`End2EndFixture.RemoveVideoGameReferencesAsync`); with leftovers present the local lookup answers and they pass with the provider escalation disabled.
    **A lower unattended-link rate here is the design, not a regression** (owner's call): only a provably right match is saved, never one an admin would have had to guess at, and a user typing the correct title and year is a legitimate part of the contract for an immediate match.
    So when matching looks too strict, improve the *queries* - ask more ways, read a deeper pool, rank better - rather than loosening what counts as identity. Video game matching is the trickiest domain here and the title plus the year is all there is to be sure with.
  - **No video game client sends the year as a server-side filter**, though a live probe confirmed Apicalypse accepts `where first_release_date` alongside `search`: that is the "an optional narrowing parameter must never silently zero out results a broader search would find" trap, and RAWG's `&dates=` was a real instance of it (a game whose RAWG date sits a year either side of what a tenant typed was absent, not ranked lower).
    Being wrong about a year costs a place in the list, never the result.
  - It is deliberately **not** used by `FindAdoptionCandidatesAsync`'s first rung, which keeps calling `FindGamesByExactTitleAsync` directly for its *complete*, uncapped answer - truncating that one would turn "several candidates, don't guess" into "exactly one, adopt it" purely by cutting the list short.
...
Every use now wraps an `<Icon>` (an inline `<svg>`), and an inline-block box aligns that on the text baseline - so the box gains the font's descender space, stands taller than the icon, and `transform-origin: 50% 50%` lands below the icon's real centre, which makes it orbit rather than spin (measured in Chromium: a 21px box around a 17px icon, pivoting 2px low - reported by the owner as "the refresh icon is not centered").
`inline-flex` makes the svg a flex item so the box hugs it exactly, and it changes nothing where the spinner sits inline in text.
...
- `test/BlazorApp.PlaywrightTests` - Playwright e2e (`Microsoft.Playwright.Xunit.v3`'s `PageTest`, plain facts, no Gherkin; see `docs/playwright-e2e-tests-plan.md`). Every test self-skips unless `E2E_ENABLED=true`, so a plain `dotnet test`
  ...
  Every seeded entry is removed by external id at the end of the test, as is every dismissal it records; only the two `Add` cases use a real provider id (TMDB 278, IGDB 72 - adding resolves the reference from the exact provider id, which a synthetic id can't).
  Its list behaviours (dismiss, "Load more", the automatic top-up a removal triggers) are `[Theory]`s over **movies and video games**, since those two differ in more than the page: the video game tab is member-only behind an `AuthorizeView`, its ranking is IGDB's rather than TMDB's, and its dismissals are recorded in IGDB's id space.
  A domain's entry is seeded under **every** ranking `ExploreRankings.Rankings` declares for it (video games have one per IGDB rating source), because which one the page reads follows the admin-selected primary rating source - a stored setting no test controls, so seeding the guessed-at one breaks on a database where the other was picked.
