...
`AuthenticationController` itself still has no automated coverage: token verification failure, cookie issuance via `HttpContext.SignInAsync`, and the "a refresh cannot swap the signed-in identity" check are only manually verified.
