...
Still uncovered: `Refresh`'s "a valid token for a *different* Firebase user cannot swap the signed-in identity" check, since proving it needs a second real Firebase user and the fixture only ever holds one signed-in identity per run.
