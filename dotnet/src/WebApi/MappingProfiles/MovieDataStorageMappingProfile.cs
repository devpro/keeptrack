    ...
    public MovieDataStorageMappingProfile()
    {
        ...
        CreateMap<Domain.Models.MovieModel, Dal.MongoDb.Entities.Movie>()
            .ForMember(x => x.Allocine, opt => opt.MapFrom(
                x => !string.IsNullOrEmpty(x.AllocineId) ? new Dal.MongoDb.Entities.Allocine { Id = x.AllocineId } : null))
            .ForMember(x => x.Imdb, opt => opt.MapFrom(
                x => !string.IsNullOrEmpty(x.ImdbPageId) ? new Dal.MongoDb.Entities.Imdb { PageId = x.ImdbPageId } : null));
    }
}
