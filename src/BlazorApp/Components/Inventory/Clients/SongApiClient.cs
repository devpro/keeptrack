    ...
    : InventoryApiClientBase<SongDto>(http)
{
    ...
    /// <summary>
    /// The single shared "add a real track" operation -
    /// reused by both <c>AlbumDetail.razor</c>'s per-track "+ Add to playlist" and <c>PlaylistDetail.razor</c>'s "Add from album" picker,
    /// so picking the same track from two different places (or twice from the same place) reuses one <see cref="SongDto"/> instead of creating a duplicate.
    /// Relies on <c>SongRepository.GetFilter</c>'s exact-match on <see cref="SongDto.AlbumId"/>/<see cref="SongDto.TrackPosition"/> - no dedicated WebApi endpoint needed.
    /// </summary>
