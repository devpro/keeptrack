...
/// <summary>
/// Shared SVG axis-drawing primitives for the app's hand-rolled charts (no charting library dependency for a
/// handful of small trend/bar charts). Extracted from <c>CarDetail.razor</c> so <c>HouseDetail.razor</c>'s own
/// yearly cost chart doesn't duplicate the same axis geometry/arrow-marker/tick-label algorithm. Deliberately
/// limited to just axis/geometry, not the per-chart series-drawing code (line vs. bar, single vs. stacked
/// series) - that part differs enough between consumers that it stays with each page rather than being forced
/// into one over-generalized shared renderer.
/// </summary>
public static class SvgChartHelpers
{
    /// <summary>
    /// Plot geometry for one chart. Not every chart shares a single fixed viewBox: a chart rendered at full
    /// row width needs a proportionally wider viewBox than a half-width one - matching ViewWidth to actual
    /// on-screen width keeps the rendered scale (and therefore axis text/arrow/tick size) the same across
    /// every chart instead of the wider ones blowing up.
    /// </summary>
    ...
    /// <summary>Draws a graduated X/Y axis pair (arrowhead, tick marks, tick labels, axis title). Ticks are
    /// computed by the caller, since what counts as an evenly-spaced value differs between a continuous line
    /// chart and a per-bar categorical one. The Y-axis title is a plain horizontal caption above the axis
    /// rather than rotated sideways along it - fine for a multi-word label like "L/100km", but a rotated
    /// single glyph like "€" reads as a completely different, garbled character, not a sideways euro sign.</summary>
    ...
    /// <summary>Picks up to <paramref name="count"/> evenly-spaced indices from a 0-based range, always
    /// including the first and last - shared by every chart's X-axis tick placement.</summary>
