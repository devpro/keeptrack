...
{
    // Platform/State/IsOwned are filter-only members on VideoGameModel with no matching entity field -
    // see VideoGameModel.Platform/State/IsOwned's own doc comments. Ignored on both directions: as an
    // unmapped target on the read side, as an unmapped source (nothing to write) on the write side.
    [MapperIgnoreSource(nameof(VideoGameModel.Platform))]
    [MapperIgnoreSource(nameof(VideoGameModel.State))]
    [MapperIgnoreSource(nameof(VideoGameModel.IsOwned))]
    ...
    [MapperIgnoreTarget(nameof(VideoGameModel.IsOwned))]
