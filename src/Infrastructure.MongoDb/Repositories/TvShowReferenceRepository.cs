...
public class TvShowReferenceRepository(IMongoDatabase mongoDatabase, TvShowReferenceStorageMapper mapper) : ITvShowReferenceRepository
{
    ...
    public async Task<TvShowReferenceModel?> FindByTitleYearAsync(string title, int? year)
    {
        ...
        // ElemMatch requires both conditions to hold on the SAME array element, so a tenant whose recorded year genuinely differs from the document's own canonical Year still matches,
        // as long as that exact (title, year) pair was confirmed at some point (automatic resolution or admin pick).
    ...
    public async Task<TvShowReferenceModel> UpsertAsync(TvShowReferenceModel model)
    {
        ...
        // the canonical (title, year) combination is always itself a valid match, whether the caller remembered to include it
