...
/// <summary>
/// Cell-parsing helpers shared by the one-off Excel importers (<see cref="CarHistoryImportService"/>,
/// <see cref="HealthImportService"/>) - extracted rather than duplicated per importer.
/// </summary>
internal static class ExcelCellParser
{
    /// <summary>
    /// "Heure" columns are inconsistent in these personal files - some cells are real Excel time values
    /// (a fraction of a day), others plain text like "11:57" - but <c>GetFormattedString()</c> renders
    /// both the same way, so a single text parse handles both.
    /// </summary>
    ...
    /// <summary>
    /// Euro-amount columns are often Excel formulas (sums of several acts), which routinely produce
    /// floating-point noise like 36.049999999999997 - rounded to the cent so every imported price is a
    /// real, displayable amount.
    /// </summary>
