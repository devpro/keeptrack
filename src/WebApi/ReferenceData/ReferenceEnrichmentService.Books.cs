...
public partial class ReferenceEnrichmentService
{
    ...
    /// <paramref name="author"/> narrows the Open Library search considerably - without it, a common
    ...
    {
        var candidates = await openLibraryClient.SearchBooksAsync(title, year, author);
        if (candidates.Count != 1) return;
        await ResolveBookAsync(title, year, candidates[0].ExternalId);
    }

    /// <summary>
    /// Resolves a title+year to a specific Open Library work id, upserts the reference document, and
    ...
    {
        var details = await openLibraryClient.GetBookDetailsAsync(externalId)
                      ?? throw new InvalidOperationException($"Open Library work {externalId} could not be fetched.");

        var existing = await bookReferenceRepository.FindByExternalIdAsync("openlibrary", externalId)
        ...
        externalIds["openlibrary"] = externalId;

        var authorReferenceId = !string.IsNullOrEmpty(details.AuthorExternalId)
            ? await ResolvePersonReferenceIdAsync("openlibrary", details.AuthorExternalId, details.Author ?? "Unknown", null)
    ...
    /// <summary>
    /// Re-fetches a book reference from Open Library, always doing a full re-fetch when called (unlike
    /// TMDB, Open Library exposes no per-id "has this changed" endpoint, so there's no cheap pre-check to
    /// skip it) - see <see cref="RefreshTvShowReferenceAsync"/> for the shared staleness-cutoff mechanism
    /// this is invoked from. A no-op (returns unchanged) for a reference with no Open Library id or that
    /// Open Library no longer has details for.
    /// </summary>
    ...
    {
        var externalId = reference.ExternalIds.GetValueOrDefault("openlibrary");
        if (string.IsNullOrEmpty(externalId)) return (reference, false);

        var details = await openLibraryClient.GetBookDetailsAsync(externalId, cancellationToken);
        ...
        {
            reference.AuthorReferenceId = await ResolvePersonReferenceIdAsync("openlibrary", details.AuthorExternalId, details.Author ?? "Unknown", null);
        }
