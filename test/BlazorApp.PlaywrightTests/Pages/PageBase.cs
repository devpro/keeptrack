...
/// set their own <c>&lt;PageTitle&gt;</c> - every list/detail page has no title convention to assert at all,
/// confirmed against a real run: clicking a link *inside* an already-interactive <c>@rendermode InteractiveServer</c>
/// component (e.g. a list row's title link) takes a different client-side navigation path than a sidebar
/// <c>NavLink</c> click and resets <c>document.title</c> to blank when the destination has no <c>PageTitle</c>
/// of its own, whereas sidebar navigation leaves whatever title was last set untouched. Rather than fight that
/// real (if surprising) behavior, only the three pages that genuinely own a title override this.
/// </remarks>
public abstract class PageBase(IPage page)
{
    ...
    /// <summary>
    /// Waits for <paramref name="expectedResult"/> to appear, re-clicking <paramref name="trigger"/> if it
    /// hasn't within a short window. Blazor Server prerenders (static SSR) before its SignalR circuit
    /// connects and wires up <c>@onclick</c> handlers - so the very first <c>@onclick</c>-driven click after
    /// a page becomes interactive can land in that gap and silently do nothing, even though the page's
    /// "loading resolved" signal (itself baked into the same prerendered payload) already reads ready.
    /// Confirmed with a real flaky run: adding artificial slowmo made it reliable, which is itself evidence of
    /// a genuine race rather than a code bug. This is a bounded, no-blind-sleep mitigation for exactly that -
    /// plain link-based navigation (<see cref="NavigateAsync{TPage}"/>) doesn't need it, since Blazor's
    /// enhanced navigation intercepts anchor clicks independently of the interactive circuit.
    /// </summary>
