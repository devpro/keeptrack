...
public class BookSmokeTest(E2eFixture fixture) : SmokeTestBase(fixture)
{
    ...
    public async Task AddEditAndDelete_BookThroughTheList()
    {
        ...
        await detail.SetFieldAsync(detail.SeriesInput, series);

        // Round-tripping via the list (rather than a raw page reload) proves the edit persisted server-side
        // just as well - BookDetail.razor re-fetches via GetOneAsync on every navigation to it - while staying
        // on Blazor's enhanced (SPA) navigation throughout, which a raw reload would drop out of.
