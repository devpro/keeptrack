...
/// </summary>
public class TvShowReferenceLinkingTest(KestrelWebAppFactory<Program> factory) : DatabaseTestBase(factory)
{
    [Fact]
    public async Task SetReferenceLinkAsync_UpdatesEveryMatchingTenantsShow_ButNotOthers()
    {
        ...
        var tenantAShow = await CreateShowAsync(repository, new TvShowModel { OwnerId = "reference-link-tenant-a", Title = title, Year = year });
        // different casing, different tenant: the match is case-insensitive and crosses tenants by design
        var tenantBShow = await CreateShowAsync(repository, new TvShowModel { OwnerId = "reference-link-tenant-b", Title = title.ToUpperInvariant(), Year = year });
        var differentYearShow = await CreateShowAsync(repository, new TvShowModel { OwnerId = "reference-link-tenant-a", Title = title, Year = year + 1 });
        var alreadyLinkedShow = await CreateShowAsync(repository, new TvShowModel
        {
    ...
    public async Task FindDistinctUnresolvedTitleYearsAsync_ReturnsDistinctUnlinkedTitleYearPairs()
    {
        ...
        await CreateShowAsync(repository, new TvShowModel { OwnerId = "unresolved-tenant-a", Title = title, Year = 2022 });
        await CreateShowAsync(repository, new TvShowModel { OwnerId = "unresolved-tenant-b", Title = title, Year = 2022 });
        await CreateShowAsync(repository, new TvShowModel
        {
    ...
    private async Task<TvShowModel> CreateShowAsync(ITvShowRepository repository, TvShowModel show)
    {
        var created = await repository.CreateAsync(show);
        TrackCleanup(() => repository.DeleteAsync(created.Id!, show.OwnerId));
        return created;
    }
}
